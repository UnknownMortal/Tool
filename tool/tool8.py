...
async def replyramd(client: Client, message: Message):
    tool = await message.reply_text("Processing.....")
    text_ = get_text(message)
    user, reason = get_user(message, text_)
    failed = 0
    if not user:
        await tool.edit("Reply To User Or Mention To Activate Replyraid !!")
        return
    try:
        userz = await client.get_users(user)
    except:
        await tool.edit(f"404 : User Doesn't Exists In This Chat !")
        return
    if not reason:
...
async def dreplyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await tool.edit("Reply To User Or Mention To Deactivate Replyraid...")
        return
    try:
        userz = await client.get_users(user)
    except:
        await tool.edit(f"404 : User Doesn't Exists!")
        return
    mee= await client.get_me()
    if userz.id == mee.id:
