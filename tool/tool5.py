...
from folder.basic import edit_or_reply, get_text
...
async def rlogo(client: Client, message: Message):
    ...
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
            photo=file_name,
            caption="Made by using Inflex M8N Tool",
            reply_to_message_id=message.reply_to_message.message_id,
        )
    else:
        await client.send_photo(
            message.chat.id, photo=file_name, caption="Made by using Inflex M8N Tool"
        )
